...
<p align="justify">
La tercera clase creada para este código lleva el nombre de **_'SistemaGestion'_**. Igual que la clase **_'Paciente'_**, esta clase es heredada de la clase **_'Estudio'_**, esto con el fin de trabajar con los mismos atributos y teniendo en cuenta que se quiere controlar la totalidad del sistema con los métodos d ela clase estudio, por lo tanto se genera una primera relación entre clases por medio de esta herencia. Además es importante apuntar que a clase **_'SistemaGestion'_** tiene una lista para almacenar específicamente los ID de los pacientes que se encuentran almacenados al momento de utilizar sus métodos. De manera general, esta clase se compone de dos métodos que son llamados dentro de la clase **_'Estudio'_**, estos generan un menú para el manejo de estudios (añadir,consultar y eliminar) que se controla o gestiona con los ID de los pacientes, además también muestra información del paciente que se consulta y por medio de un último método, muestra información de las imágenes mostradas por medio de nilearn. 

- Inicialmente, como ya se mecionó, esta clase tiene una lista llamada **_'id_list'_** donde se almacenan únicamente los ID de cada paciente que se registra, esto se logra realizar mediante otro método creado desde la clase **_'Paciente'_**, evidenciando aquí una asociación entre ambas clases.

- **_'Metodo _manejoEstudios()'_**: Importante resaltar en un principio que este método se encuentra protegido. En cuanto a su funcionalidad este método ofrece un menú inicial que ofrece las tres opciones principales para el manejo y la gestión de estudios.
  1. Si se elige añadir estudio, se pide que ingrese la ruta del nuevo estudio que se quiere añadir, con esto, el método procede a leer esta ruta para cargar el estudio por medio del método dicom **_'dcmread'_** y obtiene el iD del paciente utilizando una imagen en especifico de la carpeta anteriormente ya cargada. Con este ID se procede a utilizar condicionales para hacer un proceso de comprobación para ver si el paciente o el estudio ya se encuetran registrados, reutilizando la lógica del código de comprobación de la clase **_'Paciente'_** ya antes descrita, con esto, una vez se comprueba cual es el estado de registro del estudio, se procede a añadir en el diccionario donde se almacena toda la información de los pacientes con sus respectivos estudios, diccionario que se encuentra en la clase **_'Paciente_**.
  2. Si se elige la opción de consultar información de un estudio, el método lo que hace en primera instancia es presentar los iD de los pacientes que se encuentran registrados hasta el momento para dar a elección sobre cuál de todos se desea consultar, posterior a la elección del paciente, despliega nuevamente la lista de los estudios correspondientes a este paciente en especifico y permite elegir que estudio en específco se quiere consultar. Cuando se realizan las elecciones, el método accede a la carpeta del estudio seleccionado y toma una de sus imagenes para extraer la información del estudio y del paciente, estos filtros de informacion se realizan utilizando el método **_'.dir'_** e ingresando las palabras de intéres, que en este caso son 'Study' y 'Patient'. Por último se muestra la información de manera organizada.
  3. Si se elige la opción de eliminar estudio, el método lo que hace en primera instancia es presentar los iD de los pacientes que se encuentran registrados hasta el momento para dar a elección sobre cuál de todos se desea eliminar un estudio, posterior a la elección del paciente, despliega nuevamente la lista de los estudios correspondientes a este paciente en especifico y permite elegir que estudio en específco se quiere eliminar. Cuando se realizan las elecciones, el método accede al diccionario donde se encuentra guardada la información de los pacientes y utilizando el iD como clave y el número del estudio seleccionado como valor, se elimina este valor por medio del método **_'.remove'_**.
  4. Si se elige la opción de Salir, el método interrumpe el funcionamiento del código de manera correcta.

- Importante anotar que los menús e inputs presentes en este método se encuentran respaldados con **_'try'_**,**_'except'_** y con **_'raise'_** para evitar errores por las entradas en los inputs.

- **_'Metodo infoImagen()'_**: Este método se creo para ser llamado en la parte de visualización de imágenes con Nilearn. Lo que hace este método es recibir una ruta de un estudio ya convertido a nifti (esta ruta ya está dado cuando se inicializa la clase **_'Estudio'_**), la lee y carga la imagen respectiva, posteriormente utilizando el método **_'.shape'_** extrae  la información de la forma de la imagen (pixeles y cortes) y la presenta de manera organizada.

## **Paso 6 - Explicación e implementación de las pruebas'**
<p align="justify">
Las pruebas que permiten visualizar que todo el sistema funciona de forma correcta ya se encuentran planteadas dentro del código, pero de igual manera se pueden plantear a gusto del usuario teniendo en cuenta lo siguiente: 

- Importante que en un inicio se cargue al menos un estudio llamando la clase estudio y entregandole la ruta relativa del estudio y la ruta de la carpeta donde se está trabajando. 

- Si se quiere visualizar bien como funciona la opción de añadir estudio, se recomienda no agregar de manera manual todos los estudios, para observar mediante el método 'informacionEstudio' y la opción 1, como se añade un estudio nuevo. 

- Para visualizar algún estudio, primero se debe realizar la conversión de al menos un estudio a formato nifti para poder entregar la ruta de esta nueva carpeta de datos convertidos al método 'visualizarNilearn' y que este funcione de manera efectiva. Esta conversión de datos también se realiza por medio de un método: 'convertirEstudio'.

- Al correr la celda del código con las pruebas, siempre se usa al final el método de 'verDiccionario', esto con el fin de lograr ver la efectividad del código con cada acción que se realice y los cambios que se hacen dentro del diccionario que maneja la información de los estudios y los pacientes. 

- Por último, para probar el método de anonimización, se toma cualquiera de las pruebas por medio de cuales se cargan los estudios y se aplica este método. Agregando un par de líneas de código que permiten evidenciar que el nombre del paciente si se anonimiza.
