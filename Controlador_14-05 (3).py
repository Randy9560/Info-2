...
class Ventana(QMainWindow):
    def __init__(self): #Constructor de la clase
        ...
        loadUi(r'C:\Users\DRA01\Desktop\Info-2\Unidad 4\Ejemplo_login\VentanaLogin.ui',self)
        self.setup()
    ...
    def setup(self): # Llama al método setup, que está definido más adelante en la clase.
        ...
        self.registrar.clicked.connect(self.accion_registrar)
    ...
    def recibir_info(self, usuario,contraseña):
        resultado = self.__controlador.agregar_usuario(usuario,contraseña)
        if resultado == True:
            self.msj.setText("Este usario ya existe")
        else:
            self.msj.setText("Usario ingresado exitosamente")   
            
class ventanaEmergente(QDialog):
    def __init__(self,ppal = None): #Constructor de la clase
        QDialog.__init__(self,ppal)
        loadUi(r'C:\Users\DRA01\Desktop\Info-2\Unidad 4\Ejemplo_login\ventana_registro.ui',self)
        self.setup()
        self.parent = ppal
        

    def setup(self):
        self.botonesresgistro.accepted.connect(self.coneccionRegistro())
        self.botonesresgistro.rejected.connect(lambda:self.close())
     
    def coneccionRegistro(self):
        usuario = self.usuinfo.text()
        password = self.contrainfo.text()
