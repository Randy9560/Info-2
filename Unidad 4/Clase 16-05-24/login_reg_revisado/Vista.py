...
class Ventana(QMainWindow):
    def __init__(self): #Constructor de la clase
        ...
        loadUi(r'C:\Users\DRA01\Desktop\Info-2\Unidad 4\Clase 16-05-24\login_reg_revisado\VentanaLogin.ui',self)
        self.setup()
    ...
    def accion_registrar(self):
        ventana_emergente=ventanaEmergente(self) #Aqui el error es el self
        ventana_emergente.show()

class ventanaEmergente(QDialog):
    def __init__(self, ppl=None):  # Añadir el argumento parent
        super().__init__(ppl)
        loadUi(r'C:\Users\DRA01\Desktop\Info-2\Unidad 4\Clase 16-05-24\login_reg_revisado\ventana_secundaria.ui',self)
        self.setup()
        self.parent = ppl
    ...
    def conectarRegistro(self):
        ...
        self.contasena.setValidator(QRegExpValidator(QRegExp(r'[0-9]+')))
        usuario = self.usuario.text()
        password = self.contasena.text()

        print(self.usuario.hasAcceptableInput(),self.contasena.hasAcceptableInput())

        if self.usuario.hasAcceptableInput() and self.contasena.hasAcceptableInput():
