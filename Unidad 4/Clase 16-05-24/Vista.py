...
class ventanaEmergente(QDialog):
    ...
    def registro(self):
        ...
        self.campo_password.setValidator(QRegExpValidator(QRegExp(r'[0-9]{1,3}')))
        usuario = self.campo_usuario.text()
        password = self.campo_password.text()

        if self.campo_usuario.hasAcceptableInput() and self.campo_password.hasAcceptableInput():
